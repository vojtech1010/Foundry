...
A bounded same-session artifact repair may occur after a settled role turn.
It keeps the role-attempt identity and canonical artifact path, records the
validation errors, pre-repair artifact hash, repair prompt, and new settled
observation, and revalidates the result. Repair may rewrite only the artifact;
it cannot change project code, tests, Git state, or application data. It is not
a new implementation attempt and cannot fabricate evidence for a failed check.

Unchanged skeletons, malformed output, session mismatch, sequence regression,
or ambiguous submission cannot be accepted as success. A repair that fixes
unknown fields can still fail semantic validation and require a bounded retest.
...
The future protocol for applying a person's GitHub decision is intentionally
unspecified. Recovery must not infer it from comments, approvals, closure, or
merge.
